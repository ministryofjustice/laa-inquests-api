...
class TestGetOfficesByCodes:
    def test_returns_offices_lookup_from_successful_api_response(self, adapter):
        mock_response = MagicMock()
        mock_response.json.return_value = [
            {
                "firm": None,
        ...
        with patch("httpx.post", return_value=mock_response):
            result = adapter.get_offices_by_codes(["001", "002"])

        assert result["001"]["officeName"] == "Office One"
        assert result["001"]["firmOfficeId"] == 10
        assert result["002"]["officeName"] == "Office Two"
        assert result["002"]["ccmsFirmOfficeId"] == 21

    def test_calls_correct_url_with_office_codes_payload(self, adapter):
        mock_response = MagicMock()
        mock_response.json.return_value = []

        with patch("httpx.post", return_value=mock_response) as mock_post:
            adapter.get_offices_by_codes(["001", "002"])

        mock_post.assert_called_once_with(
            "https://example.com/api/v1/provider-offices",
            json={"officeCodes": ["001", "002"]},
            headers={"X-Authorization": "test-key"},
        )

    def test_returns_empty_dict_when_office_codes_is_empty(self, adapter):
        result = adapter.get_offices_by_codes([])

        assert result == {}

    def test_ignores_rows_with_missing_office_code(self, adapter):
        mock_response = MagicMock()
        mock_response.json.return_value = [
            {
                "firm": None,
                "offices": [
                    {"firmOfficeCode": "001", "officeName": "Office One"},
                    {"firmOfficeCode": None, "officeName": "No Code"},
                    {"firmOfficeCode": "003", "officeName": None},
                ],
            }
        ]

        with patch("httpx.post", return_value=mock_response):
            result = adapter.get_offices_by_codes(["001", "003"])
    ...
    def test_raises_provider_details_retrieval_error_on_http_error(self, adapter):
        ...
        with (
            ...
            adapter.get_offices_by_codes(["001"])

    def test_raises_provider_details_retrieval_error_on_non_list_response(
        self, adapter
    ):
        mock_response = MagicMock()
